...
export const POUR_CELL_ADD_CAP = 0.015;
/** Point-source increment per step. */
export const SOURCE_CELL_ADD_CAP = 0.012;
/** Sparse rain drop increment. */
export const RAIN_CELL_ADD_CAP = 0.0048;

const WET_EPS = 0.00035;
const DESPIKE_RATIO = 1.45;
const DESPIKE_PAD = 0.007;
const DRY_WEIGHT = 0.055;
const BLUR_PASSES = 2;
const TEMPORAL_WATER = 0.4;
const TEMPORAL_FLOW = 0.5;
const FLOW_SOFT_KNEE = 0.12;
const FLOW_THIN = 0.007;
const KERNEL = [1, 2, 4, 2, 1] as const;
...
export function prepareDisplayMaps(
  ...
  for (let i = 0; i < n; i++) {
    const w = outWater[i];
    outWater[i] = w > WET_EPS && Number.isFinite(w) ? w : 0;
  }
